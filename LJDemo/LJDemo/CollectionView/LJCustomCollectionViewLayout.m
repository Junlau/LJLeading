...
@interface LJCustomCollectionViewLayout ()
    ...
    for (NSArray *section in self.itemAttributes) {
        [attributes addObjectsFromArray:[section filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UICollectionViewLayoutAttributes *evaluatedObject, NSDictionary *bindings) {
            if (CGRectIntersectsRect(rect, [evaluatedObject frame])) {
                if (evaluatedObject.indexPath.section == 0) {
                    evaluatedObject.frame = CGRectMake(evaluatedObject.frame.origin.x, self.collectionView.contentOffset.y, evaluatedObject.frame.size.width, evaluatedObject.frame.size.height);
                }
                if (evaluatedObject.indexPath.item == 0) {
                    evaluatedObject.frame = CGRectMake(self.collectionView.contentOffset.x, evaluatedObject.frame.origin.y, evaluatedObject.frame.size.width, evaluatedObject.frame.size.height);
                }
                return YES;
            } else {
                return NO;
...
- (CGSize)sizeForItemWithColumnString:(NSString *)columnString
{
    
    NSString *text = columnString;
    
    CGSize size = [text sizeWithAttributes: @{NSFontAttributeName:[UIFont fontWithName:@"HelveticaNeue" size:15]}];
    return CGSizeMake(ceilf(size.width) + 16, 30);
}
...
- (void)calculateItemsSize
{
    ...
    for (NSInteger section = 0; section < self.dataArray.count; section++) {
        NSArray *rows = self.dataArray[section];
        for (NSUInteger index = 0; index < rows.count; index++) {
            if (columns.count <= index) {
                [columns addObject:rows[index]];
            } else {
                NSString *columString = columns[index];
                NSString *rowsString = rows[index];
                if (!columString || columString.length < rowsString.length) {
    ...
    for (NSUInteger index = 0; index < columns.count; index++) {
        if (self.itemsSize.count <= index) {
            CGSize itemSize = [self sizeForItemWithColumnString:columns[index]];
